  ...
  function uiSortable() {
    ...
    $('.acc-container .acc-btn').on("mousedown", function (e) {
      if (e.which === 1) {
        ...
        timerUiItem = setInterval(function () {
          ...
          if (fullTime > 400) {
            $(e.currentTarget).parent().children('.acc-content').css({
              'position': 'absolute',
              'display': 'none',
              'overflow': 'inherit'
            });
    ...
    $('ul.acc-container').sortable({
      ...
      sort: function (e, ui) {
        ...
        ui.item.css({
          'height': ui.item.children('.acc-btn').outerHeight()
        });
        ui.placeholder.css({
          'height': ui.item.children('.acc-btn').outerHeight() + 4
        });
      },
      stop: function (e, ui) {
        clearInterval(timerUiItem);
