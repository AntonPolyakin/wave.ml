...
jQuery(document).ready(function () {
	jQuery('.sub-menu-dropdown').scrollbar();
});
...
/*switcher*/
document.addEventListener("DOMContentLoaded", function () {
	var switcher = document.querySelector('.menu-hasdropdown');

	function switchMenuCategory() {
		var menuArtists = document.querySelectorAll('.menu-artists');
		var menuPlaylists = document.querySelectorAll('.menu-playlists');
		if (switcher.children[1].children[0].textContent == "Playlists") {
			switcher.children[1].children[0].textContent = "Artists";
			switcher.children[2].innerHTML = '<i class="fas fa-users"></i>';
			for (let i = 0; i < menuArtists.length; i++) {
				menuArtists[i].classList.remove('hided');
				menuPlaylists[i].classList.add('hided');
			}
		} else {
			switcher.children[1].children[0].textContent = "Playlists";
			switcher.children[2].innerHTML = '<i class="fas fa-music"></i>';
			for (let i = 0; i < menuArtists.length; i++) {
				menuArtists[i].classList.add('hided');
				menuPlaylists[i].classList.remove('hided');
			}
		}
	}
	switcher.addEventListener('click', switchMenuCategory);
});
/*end of switcher*/
...
$(document).ready(function () {
	...
	function animateHeader() {
		...
		setTimeout(function () {
			...
			$('.player-controls').addClass('scrolled');
		}, 450);
	...
	$(window).scroll(function () {
		if ($(this).scrollTop() > playerTop) {
			...
			$('.player-controls').removeClass('scrolled');
...
$(document).ready(function () {
	...
	$(".input-style input").focus(function () {
		$(this).offsetParent().offsetParent().find(".style").animate({
			width: "100%"
		}, 500);
		$(this).offsetParent().offsetParent().find("button").toggleClass("active");
	});
	$(".input-style input").blur(function () {
		$(this).offsetParent().offsetParent().find(".style").css({
			width: "0%"
		});
		$(this).offsetParent().offsetParent().find("button").toggleClass("active");
	});
